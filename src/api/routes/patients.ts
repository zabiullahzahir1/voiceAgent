...
export async function registerPatientRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/patients', async (request, reply) => {
    const { rows, total } = patients.listPatients(request.query);
  ...
  app.get<{ Params: { id: string } }>('/patients/:id', async (request, reply) => {
    const patient = patients.getPatient(assertUuid(request.params.id));
  ...
  app.post(
    ...
    async (request, reply) => {
      const patient = patients.createPatient(request.body);
      return reply
        .status(201)
  ...
  app.put<{ Params: { id: string } }>(
    ...
    { preHandler: requireApiToken },
    async (request, reply) => {
      const patient = patients.updatePatient(assertUuid(request.params.id), request.body);
      return reply.status(200).send(ok(patients.toApiView(patient)));
    },
  );
  ...
  app.delete<{ Params: { id: string } }>(
    ...
    { preHandler: requireApiToken },
    async (request, reply) => {
      const patient = patients.deletePatient(assertUuid(request.params.id));
      return reply
        .status(200)
